...
public class GrafoNDP extends Grafo{
	...
	public GrafoNDP(String path) throws FileNotFoundException {
		...
		dimensionPesosAristas = (cantNodos * cantNodos - cantNodos) / 2;
		
		pesosAristas = new int[dimensionPesosAristas];
		...
		for (int i = 1; i < cantNodos; i++) {
			...
			while (j < cantNodos) {
				if (nodo != j) {
					...
					if (grafo.hayArista(indice) && nodosColoreados[nodo] == nodosColoreados[j]) {
						color++;
						if (color > colorMax)
							colorMax = color;
						nodosColoreados[nodo] = color;
